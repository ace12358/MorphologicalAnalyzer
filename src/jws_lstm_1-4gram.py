...
def make_vocab():
    ...
    for f in [train_file, test_file]:
        for line in open(f):
            ...
            for i in range(window//2 + ngram_size-1):
                sent.append('</s>')
                sent.insert(0,'<s>')
            for i in range(ngram_size-1, len(sent)-(ngram_size-1)+2):
                uni_gram = sent[i]
                bi_gram = sent[i-1] + sent[i]
                tri_gram = sent[i-2] + sent[i-1] + sent[i]
                four_gram = sent[i-3] + sent[i-2] + sent[i+1] + sent[i]
                ...
                if four_gram not in vocab:
                    vocab[four_gram] = len(vocab)
    ...
    for one in ['hiragana','katakana','kanji','alphabet','number','other','<s>','</s>']:
        for two in ['hiragana','katakana','kanji','alphabet','number','other','<s>','</s>']:
            ...
            for three in ['hiragana','katakana','kanji','alphabet','number','other','<s>','</s>']:
                ...
                for four in ['hiragana','katakana','kanji','alphabet','number','other','<s>','</s>']:
                    char_type2id[one+two+three+four] = len(char_type2id)
    return char_type2id
...
def init_model(vocab_size, char_type_size):
    model = FunctionSet(
        ...
        hidden1=F.Linear(window * (embed_units + char_type_embed_units)*ngram_size + hidden_units, hidden_units),
        i_gate=F.Linear(window * (embed_units + char_type_embed_units)*ngram_size + hidden_units, hidden_units),
        f_gate=F.Linear(window * (embed_units + char_type_embed_units)*ngram_size + hidden_units, hidden_units),
        o_gate=F.Linear(window * (embed_units + char_type_embed_units)*ngram_size + hidden_units, hidden_units),
...
#    dict_vec = chainer.Variable(np.array([[L1,L2,L3,L4,R1,R2,R3,R4,I1,I2,I3,I4]], dtype=np.float32))
    ...
    # make char vector 
        ...
        four_gram = x[target+s_num-3+i] + x[target+s_num-2+i] + x[target+s_num-1+i] + x[target+s_num+i]
        # char2id
        uni_gram_id = char2id[uni_gram]
        bi_gram_id = char2id[bi_gram]
        tri_gram_id = char2id[tri_gram]
        four_gram_id = char2id[tri_gram]
        # id 2 embedding
    ...
    # make char type vector 
        ...
        four_gram_type = make_char_type(x[target+s_num-3+i]) + make_char_type(x[target+s_num-2+i]) + make_char_type(x[target+s_num+i] + make_char_type(x[target+s_num-2+i]))
        # chartype 2 id
        ...
        four_gram_type_id = char_type2id[four_gram_type]
        ...
        four_gram_type_vec = model.char_type_embed(get_onehot(four_gram_type_id))
        ...
        char_type_vecs.append(four_gram_type_vec)
...
if __name__ == '__main__':
    ...
    ngram_size = int(ini.get('Parameters', 'ngram_size'))
